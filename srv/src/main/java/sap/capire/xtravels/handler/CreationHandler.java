...
class CreationHandler implements EventHandler {
  ...
  CreationHandler(PersistenceService db) {
    this.db = db;
  }
  ...
  void calculateTravelId(final Travels travel) {
    var result = db.run(Select.from(TRAVELS).columns(t -> t.ID().max().as("maxID")));
  ...
  void calculateBookingPos(Bookings_ ref, final Bookings booking) {
    var result = db.run(Select.from(ref).columns(t -> t.Pos().max().as("maxPos")));
